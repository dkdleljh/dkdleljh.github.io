---
layout: page
title: Projects
permalink: /projects/
---

# Projects

현재 다루는 프로젝트를 한눈에 볼 수 있도록 정리한 페이지입니다. 아래에는 직접 소개하는 핵심 프로젝트를 먼저 두고, 맨 아래에는 GitHub 저장소 전체 목록을 자동으로 동기화해 보여 줍니다.

## 핵심 프로젝트

### Autonomous Open Problem Lab

수학 난제 후보를 수집하고, 정규화하고, 검증하고, 형식화와 논문화까지 이어 주는 무인 연구 파이프라인입니다.

- 링크: <https://github.com/dkdleljh/autonomous_open_problem_lab>
- 초점: 자동화 연구 워크플로우, 감사 로그, 게이트 기반 품질 관리

### My-Second-Brain (Private)

Obsidian Vault를 기반으로 기록, 연결, 실행 루프를 실제 운영 시스템처럼 관리하는 개인 지식 프로젝트입니다.

- 목표: 검색 가능한 기록 + 자동화된 운영
- 운영 원칙: 성공은 침묵, 실패만 알림 / 민감정보는 외부 공개 제외

### 이 블로그

GitHub Pages와 Jekyll 기반으로 운영하는 개인 브랜딩 사이트입니다. 단순 소개 페이지가 아니라 프로젝트 인덱스와 운영 흔적을 함께 쌓는 공간으로 사용하고 있습니다.

- 링크: <https://github.com/dkdleljh/dkdleljh.github.io>
- 역할: 포트폴리오, 링크 허브, 프로젝트 인덱스

## 공개 프로젝트 묶음

### 자동화와 운영

- [UptimeGuardian](https://github.com/dkdleljh/UptimeGuardian) — Go 기반 업타임 모니터링 및 전환 기반 웹훅 알림 서비스
- [HAF9_Studio](https://github.com/dkdleljh/HAF9_Studio) — 프로그래밍 가능한 공간 운영 체제 실험
- [HermesForge](https://github.com/dkdleljh/HermesForge) — 가능역 기반 엔진 실험
- [Synapse_Engine](https://github.com/dkdleljh/Synapse_Engine) — 시나리오 평가 및 패치 생성 CLI 도구

### 트레이딩과 데이터 수집

- [kis-orb-vwap-bot](https://github.com/dkdleljh/kis-orb-vwap-bot) — KIS 기반 ORB/VWAP 모듈형 트레이딩 봇
- [kis_adaptive_vb_bot](https://github.com/dkdleljh/kis_adaptive_vb_bot) — ETF 페어 기반 Adaptive Volatility Breakout 봇
- [upbit_bot](https://github.com/dkdleljh/upbit_bot) — 업비트 현물 자동매매 봇
- [goyoonjung_photo_collector](https://github.com/dkdleljh/goyoonjung_photo_collector) — 사진/이미지 수집·정리 자동화 도구

### 제품과 서비스

- [WeddingFlow](https://github.com/dkdleljh/WeddingFlow) — 웨딩 준비 전 과정을 관리하는 한국어 기반 웨딩 준비 SaaS
- [WeddingMap](https://github.com/dkdleljh/WeddingMap) — 전국 예식장 비교·분석 플랫폼
- [wedding-expo-scraper](https://github.com/dkdleljh/wedding-expo-scraper) — 광주광역시 웨딩박람회 일정 자동 업데이트 프로그램

### 게임과 인터랙티브 프로젝트

- [rabbit-skater](https://github.com/dkdleljh/rabbit-skater) — Rabbit Skater 안드로이드 게임 배포 패키지 저장소
- [rabbit-skater-v3](https://github.com/dkdleljh/rabbit-skater-v3) — Rabbit Skater v3 안드로이드 게임 소스코드 및 에셋 저장소
- [universe-live](https://github.com/dkdleljh/universe-live) — 관측 가능한 우주를 웹에서 탐색하는 3D 플랫폼

### 문서화와 아카이브

- [goyoonjung-wiki](https://github.com/dkdleljh/goyoonjung-wiki) — 특정 주제 중심 위키/문서화 프로젝트

## 비공개 작업

아래 저장소는 링크는 공개하지만, 권한이 없는 경우 접근할 수 없습니다.

- [My-Second-Brain](https://github.com/dkdleljh/My-Second-Brain) — 개인 지식 저장소와 무인 운영 자동화
- [Simple_Shooter](https://github.com/dkdleljh/Simple_Shooter) — 게임/그래픽 프로토타입
- [pyupbit-autotrade](https://github.com/dkdleljh/pyupbit-autotrade) — pyupbit 기반 자동매매 예제 및 학습 코드

## 관심 분야

- 자동화와 운영 시스템
- 지식관리와 개인 생산성
- 서비스 설계와 데이터 흐름
- 트레이딩 자동화
- 게임과 인터랙티브 프로토타입

## 전체 저장소 목록

<!-- BEGIN AUTO:REPO-LIST -->
### All repositories

- [dkdleljh.github.io](https://github.com/dkdleljh/dkdleljh.github.io) — Personal blog by Zenith (Public)
- [autonomous_open_problem_lab](https://github.com/dkdleljh/autonomous_open_problem_lab) — 완전 무인 자동화 수학 난제 수집·탐색·검증·논문화 시스템 (Public)
- [goyoonjung-wiki](https://github.com/dkdleljh/goyoonjung-wiki) — goyoonjung-wiki (Public)
- [goyoonjung_photo_collector](https://github.com/dkdleljh/goyoonjung_photo_collector) — 사진/이미지 수집·정리 자동화 도구 (Public)
- [HAF9_Studio](https://github.com/dkdleljh/HAF9_Studio) — Holozyme Architecture Fabric - 프로그래밍 가능한 공간 운영 체제 (Public)
- [HermesForge](https://github.com/dkdleljh/HermesForge) — HermesForge - 가능역(Possibility Field) 엔진 (Public)
- [kis-orb-vwap-bot](https://github.com/dkdleljh/kis-orb-vwap-bot) — KIS 기반 자동매매(ORB/VWAP) — KR/US 모듈형 트레이딩 봇 (Public)
- [kis_adaptive_vb_bot](https://github.com/dkdleljh/kis_adaptive_vb_bot) — PairBot: KIS Adaptive Volatility Breakout for KODEX ETF pairs (Public)
- [rabbit-skater](https://github.com/dkdleljh/rabbit-skater) — Rabbit Skater 안드로이드 게임 배포 패키지 저장소 (APK/AAB) (Public)
- [rabbit-skater-v3](https://github.com/dkdleljh/rabbit-skater-v3) — Rabbit Skater v3 안드로이드 게임 소스코드 및 에셋 저장소 (Public)
- [Synapse_Engine](https://github.com/dkdleljh/Synapse_Engine) — Synapse Engine - 시나리오 평가 및 패치 생성 CLI 도구 (Public)
- [universe-live](https://github.com/dkdleljh/universe-live) — 관측 가능한 우주를 웹에서 탐색할 수 있는 3D 플랫폼 (Public)
- [upbit_bot](https://github.com/dkdleljh/upbit_bot) — 업비트 현물 자동매매 봇 (paper/backtest/live) (Public)
- [UptimeGuardian](https://github.com/dkdleljh/UptimeGuardian) — Go 기반 업타임 모니터링 및 전환 기반 웹훅 알림 서비스 (Public)
- [wedding-expo-scraper](https://github.com/dkdleljh/wedding-expo-scraper) — 광주광역시 웨딩박람회 일정 자동 업데이트 프로그램 (Public)
- [WeddingFlow](https://github.com/dkdleljh/WeddingFlow) — 웨딩 준비 전 과정을 관리하는 한국어 기반 웨딩 준비 SaaS (Public)
- [WeddingMap](https://github.com/dkdleljh/WeddingMap) — 전국 예식장 비교·분석 플랫폼 WeddingMap (Public)

<!-- updated: 2026-04-12 03:46 +0000 -->
<!-- END AUTO:REPO-LIST -->
